"""
quantize_utils.py — per-tensor INT8 symmetric quantization for shared expert FFN.
OUTLIER-RUNTIME-003
...
def dequant_int8_matmul(
    ...
    scale:       scalar float (per-tensor quantization)
    ...
    for start in range(0, in_features, QUANT_CHUNK):
        ...
        w_chunk = weight_int8[:, start:end].float() * s   # (out_features, C)
